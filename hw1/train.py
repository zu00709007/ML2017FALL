...
for i in range(0,training_time):
	...
	for j in range(0,data_len):
		...
		inner_product = 0
		for k in range(0,18):
			inner_product += w[k] * float(x_item[j][k][0])		
		# get y - wx
		...
		# get Σ(y - wx)x 
		for k in range(0,17):
			total[k] += learning_rate * inner_product * float(x_item[j][k][0])			
		# constant
		total[17] += learning_rate * inner_product	
	# update w
	for k in range(0,18):
