...
x_item = list()
y_item = list()
# read process and split the data into x_vector(first 9 hours PM2.5) and y_vector(10th hour PM2.5) and add constant
for i in range(0, len(data)):
	for j in range(0, len(data[i]) - 9):
		x_item.append(data[i][j : j + 9] + [1])
		y_item.append(data[i][j + 9])
...
for i in range(0,training_time):
	...
	for j in range(0,data):
		...
		# constant
		total[9] += learning_rate * inner_product	
